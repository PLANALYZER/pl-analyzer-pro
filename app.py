...
# --- 1. RECUPERO STATISTICHE (Cerca in più stagioni) ---
...
if st.button("ESEGUI ANALISI"):
    with st.spinner("Sincronizzazione database..."):
        ...
        # Chiamata quote (allungata a 7 giorni per trovare più match)
    ...
    if standings and isinstance(res_odds, list) and len(res_odds) > 0:
        ...
        st.write(f"📊 Media Gol Campionato: {avg_l:.2f}")
        
        match_count = 0
        for match in res_odds:
            h_n, a_n = match['home_team'].lower(), match['away_team'].lower()
            
            # --- RICERCA FLESSIBILE (Trova Inter in Inter Milan) ---
        ...
        if match_count == 0:
            st.warning("⚠️ Quote trovate, ma i nomi delle squadre non corrispondono alla classifica.")
            st.write("Esempio nomi in classifica:", list(team_stats.keys())[:5])
    else:
        st.error("❌ Nessun match trovato per questo campionato nelle prossime ore.")
        st.info("Prova a cambiare campionato (es. Olanda 🇳🇱) per verificare la connessione.")
