...
if st.button("ESEGUI ANALISI"):
    with st.spinner("Sincronizzazione API in corso..."):
        ...
        # Chiamata Risultati (Ultimi 30 giorni per calcolare la forma reale)
        ...
        if "standings" in res_stats and "matches" in res_history:
            ...
            for match in res_odds:
                ...
                if datetime.utcnow() <= m_time <= ora_lim:
                    ...
                    if h_s and a_s:
                        # --- PUNTO 7: FORMULA FORMA REALE (Calcolata dai match) ---
                        def calc_form(t_name):
                            pts = 0
                            # Filtriamo i match finiti di questa squadra
                            t_m = [m for m in match_history if (m['homeTeam']['name'] == t_name or m['awayTeam']['name'] == t_name) and m['status'] == 'FINISHED']
                            for m in t_m[-5:]: # Ultime 5
                                win = m['score']['winner']
                                if win == 'DRAW': pts += 1
                                elif (win == 'HOME_TEAM' and m['homeTeam']['name'] == t_name) or (win == 'AWAY_TEAM' and m['awayTeam']['name'] == t_name): pts += 3
                            return pts

                        f_h = calc_form(h_s['team']['name'])
                        f_a = calc_form(a_s['team']['name'])
                        ...
                        with st.expander(f"📊 {h_n} vs {a_n} | xG: {txg:.2f}"):
                            ...
                            with c1:
                                ...
                                st.write(f"xG: {xh:.2f}")
                            with c2:
                                st.metric("FORMA OSPITE", f"{f_a}/15")
                                st.write(f"xG: {xa:.2f}")
                            with c3:
                                try:
                            ...
                            if txg > 2.6 and o25 < 1.90: st.success("🎯 CONSIGLIO: OVER 2.5 (ALTO POTENZIALE)")
                            elif txg < 2.2: st.warning("🛡️ CONSIGLIO: UNDER 2.5")
                            if xh > 0.9 and xa > 0.9: st.info("⚽ OPZIONE: GOAL")
