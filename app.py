...
st.title("⚽ ANALYZER PRO - CALCOLO ID UNIVOCO")
...
if st.button("ESEGUI ANALISI"):
    with st.spinner("Sincronizzazione ID squadre in corso..."):
        headers = {'X-Auth-Token': FOOTBALL_DATA_KEY}
        
        # 1. Recupero Classifica (per avere ID e Statistiche)
        res_stats = requests.get(f"https://api.football-data.org/v4/competitions/{league_map[scelta]}/standings", headers=headers).json()
        
        # 2. Recupero Match finiti (per la Forma)
        res_history = requests.get(f"https://api.football-data.org/v4/competitions/{league_map[scelta]}/matches?status=FINISHED", headers=headers).json()
        ...
        if "standings" in res_stats and "matches" in res_history:
            ...
            # Database squadre indicizzato per NOME (per incrocio con quote)
            ...
            for match in res_odds:
                ...
                if datetime.utcnow() <= m_time <= ora_lim:
                    ...
                    # Troviamo i dati della squadra usando una ricerca parziale nel nome
                    ...
                    if h_s and a_s:
                        # ID UNICI (Non sbagliano mai)
                        h_id = h_s['team']['id']
                        a_id = a_s['team']['id']

                        # --- FUNZIONE FORMA BASATA SU ID ---
                        def calc_form_by_id(t_id):
                            pts = 0
                            # Filtriamo solo i match dove compare l'ID della squadra
                            t_m = [m for m in match_history if m['homeTeam']['id'] == t_id or m['awayTeam']['id'] == t_id]
                            for m in t_m[-5:]: # Ultime 5 giocate
                                win = m['score']['winner']
                                if win == 'DRAW': pts += 1
                                elif (win == 'HOME_TEAM' and m['homeTeam']['id'] == t_id) or \
                                     (win == 'AWAY_TEAM' and m['awayTeam']['id'] == t_id):
                                    pts += 3
                            return pts

                        f_h = calc_form_by_id(h_id)
                        f_a = calc_form_by_id(a_id)
                        ...
                        with st.expander(f"📊 {h_n} vs {a_n} | xG: {txg:.2f}"):
                            ...
                            with col1:
                                ...
                                st.caption(f"Gol Fatti: {h_gf}")
                            with col2:
                                st.metric(f"FORMA {a_n}", f"{f_a}/15")
                                st.caption(f"Gol Fatti: {a_gf}")
                            with col3:
                                try:
                            ...
                            st.divider()
                            # LOGICA PRONOSTICO
                            if txg > 2.5 and f_h > 8: st.success("🎯 CONSIGLIO: OVER 2.5")
                            elif txg < 2.1: st.warning("🛡️ CONSIGLIO: UNDER 2.5")
                            if f_h > 11 or f_a > 11: st.info("🔥 SQUADRA IN STRISCIA POSITIVA")
